...
describe("User Testing 1", () => {
    ...
    describe("Test getAccountSchema()", () => {
        ...
        test("Ensure that password Hash was set correctly", () => {
            expect(testUser.getAccountSchema().passwordHash).toEqual(
                newUserSchema.passwordHash
            );
        });
        test("Ensure that first name was set correctly", () => {
            expect(testUser.getAccountSchema().public.firstName).toEqual(
                newUserSchema.public.firstName
            );
        });
        test("Ensure that first name was set correctly", () => {
            expect(testUser.getAccountSchema().public.lastName).toEqual(
                newUserSchema.public.lastName
            );
        });
        test("Ensure that first name was set correctly", () => {
            expect(testUser.getAccountSchema().public.customURL).toEqual(
                newUserSchema.public.customURL
            );
        });
        test("Ensure that first name was set correctly", () => {
            expect(testUser.getAccountSchema().public.profilePictureURL).toEqual(
                newUserSchema.public.profilePictureURL
            );
        });
    });
    ...
    describe("Test setAccountStatus()", () => {
        test("Ensures that Account Status will be set correctly", () => {
            expect(
                testUser.setAccountStatus(testUser.getAccountStatus())
            ).toBeDefined();
        });
    });
    ...
    describe("Test setVerficationCode()", () => {
        test("Ensures that the Verfication Code will be set correctly", () => {
            expect(
                testUser.setVerificationCode(testUser.getVerificationCode())
            ).toBeDefined();
        });
    });
    ...
    test("Ensures that tryPassword() is defined", () => {
        expect(
            testUser.tryPassword(testUser.getAccountSchema().passwordHash)
        ).toBeDefined();
    });
    ...
    describe("Test updateSavedCard()", () => {
        test("Ensures that savedCard update return true", () => {
            expect(
                testUser.updateSavedCard(testUser.getSavedCard("1232354"))
            ).toBeTruthy();
        });
    });
